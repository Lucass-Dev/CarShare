...
document.addEventListener('DOMContentLoaded', function() {
    ...
    userSelect.addEventListener('change', function() {
        ...
        fetch(url('index.php?action=signalement_get_carpoolings&user_id=' + userId))
            .then(response => {
                if (!response.ok) {
    ...
    form.addEventListener('submit', function(e) {
        ...
        if (userId && carpoolingId) {
            window.location.href = url('index.php?action=signalement&user_id=' + userId + '&carpooling_id=' + carpoolingId);
        }
    });
});
