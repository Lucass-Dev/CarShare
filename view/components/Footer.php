<!-- FOOTER START - UNIQUE INSTANCE -->
<footer class="modern-footer">
    <div class="footer-content">
        <div class="footer-section">
            <h3>Voyager en covoiturage</h3>
            <ul>
                <li><a href="index.php?action=search">Rechercher un trajet</a></li>
                <li><a href="index.php?action=offers">Tous les trajets</a></li>
                <li><a href="index.php?action=history">Historique</a></li>
            </ul>
        </div>

        <div class="footer-section">
            <h3>Informations</h3>
            <ul>
                <li><a href="index.php?action=about">Qui sommes-nous ?</a></li>
                <li><a href="index.php?action=faq">FAQ</a></li>
                <li><a href="index.php?action=contact">Nous contacter</a></li>
                <li><a href="index.php?action=cgu">Conditions générales</a></li>
                <li><a href="index.php?action=cgv">CGV</a></li>
                <li><a href="index.php?action=legal">Mentions légales</a></li>
            </ul>
        </div>

        <div class="footer-section">
            <h3>En savoir plus</h3>
            <ul>
                <li><a href="index.php?action=daily_carshare">Covoiturage au quotidien</a></li>
                <li><a href="#" data-coming-soon="Notre blog arrive très bientôt">Blog de voyage</a></li>
            </ul>
            <div class="footer-contact">
                <a href="mailto:carshare.cov@gmail.com" class="footer-email">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/>
                        <polyline points="22,6 12,13 2,6"/>
                    </svg>
                    carshare.cov@gmail.com
                </a>
            </div>
        </div>
    </div>

    <div class="footer-bottom">
        <p>&copy; <?php echo date('Y'); ?> CarShare by HexTech. Tous droits réservés.</p>
    </div>
</footer>
<!-- FOOTER END - UNIQUE INSTANCE -->
