...
<div class="page-container">
    ...
    <?php if (isset($_GET['success']) && $_GET['success'] === 'trip_created'): ?>
        <div style="background: linear-gradient(135deg, #10b981 0%, #059669 100%); color: white; padding: 16px 20px; border-radius: 12px; margin-bottom: 24px; display: flex; align-items: center; gap: 12px; box-shadow: 0 4px 12px rgba(16, 185, 129, 0.3);">
            ...
            <div>
                ...
                <p style="margin: 4px 0 0 0; opacity: 0.9; font-size: 14px;">Votre trajet est maintenant disponible pour les réservations.</p>
            </div>
        </div>
    <?php endif; ?>
    ...
    <section class="trips-section">
        ...
        $hasUpcoming = false;
        foreach ($carpoolingsWithBookings as $trip): 
            // Afficher si le trajet n'est pas encore passé (peu importe les places disponibles)
            if (strtotime($trip['start_date']) > time()): 
                $hasUpcoming = true;
                $tripId = $trip['trip_id'] ?? $trip['id'];
                $noSeatsLeft = (int)$trip['available_places'] === 0;
        ?>
            <div class="trip-card trip-card--upcoming">
                <?php if ($noSeatsLeft): ?>
                <div class="trip-card__badge trip-card__badge--full" style="background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%);">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <circle cx="12" cy="12" r="10"/>
                        <line x1="15" y1="9" x2="9" y2="15"/>
                        <line x1="9" y1="9" x2="15" y2="15"/>
