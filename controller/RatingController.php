...
class RatingController
{
    ...
    public function render(): void
    {
        ...
        if (!$user) {
            header('Location: ' . url('index.php?action=rating&error=user_not_found'));
            exit;
        }
    ...
    public function submit(): void
    {
        ...
        // Validation
        if (!$userId) {
            header('Location: ' . url('index.php?action=rating&error=missing_data'));
            exit;
        }

        // Check that user is not rating themselves
        if ($userId === $_SESSION['user_id']) {
            $redirectUrl = url('index.php?action=rating&user_id=' . $userId . '&error=self_rating');
            if ($carpoolingId) {
        ...
        if ($carpoolingId === null) {
            ...
            if ($inferredCarpoolingId !== null) {
                ...
                $redirectUrl = url('index.php?action=rating&user_id=' . $userId . '&error=no_trip');
                header('Location: ' . $redirectUrl);
                exit;
            }
        }
        ...
        if ($result) {
            $redirectUrl = url('index.php?action=rating&user_id=' . $userId . '&success=1');
            if ($carpoolingId) {
                $redirectUrl .= '&carpooling_id=' . $carpoolingId;
            }
            header('Location: ' . $redirectUrl);
        } else {
            $redirectUrl = url('index.php?action=rating&user_id=' . $userId . '&error=save_failed');
            if ($carpoolingId) {
                $redirectUrl .= '&carpooling_id=' . $carpoolingId;
            }
            header('Location: ' . $redirectUrl);
