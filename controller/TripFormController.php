...
class TripFormController
{
    ...
    public function submit(): void
    {
        ...
        // Get checkbox options (animals, smoking, luggage)
        $petsAllowed = isset($_POST['animals']) ? 1 : 0;
        $smokerAllowed = isset($_POST['smoking']) ? 1 : 0;
        $luggageAllowed = isset($_POST['luggage']) ? 1 : 0;
        ...
        if (!empty($errors)) {
            ...
            header('Location: ' . url('index.php?action=create_trip&error=1'));
            exit;
        }

        // Create datetime string (date saisie par l'utilisateur = date du trajet, PAS date de création)
        $startDate = $date . ' ' . ($time ?: '00:00:00');

        // Prepare data for insertion
        $tripData = [
            'provider_id' => $_SESSION['user_id'],
            'start_date' => $startDate,  // Date du TRAJET (pas date de création)
            'price' => $price,
        ...
        if ($result) {
            ...
            // Rediriger vers mes trajets avec message de succès
            header('Location: ' . url('index.php?action=my_trips&success=trip_created&trip_id=' . $result));
        } else {
    ...
    public function renderEdit(): void
    {
        ...
        if ($tripId <= 0) {
            header('Location: ' . url('index.php?action=my_trips&error=invalid_trip'));
            exit;
        }
        ...
        if (!$trip || $trip['provider_id'] != $_SESSION['user_id']) {
            header('Location: ' . url('index.php?action=my_trips&error=unauthorized'));
            exit;
        }

        // Check if trip is in the future
        if (strtotime($trip['start_date']) <= time()) {
            header('Location: ' . url('index.php?action=my_trips&error=past_trip'));
            exit;
        }
    ...
    public function submitEdit(): void
    {
        ...
        if (!empty($errors)) {
            $_SESSION['trip_form_errors'] = $errors;
            $_SESSION['trip_form_data'] = $_POST;
            header('Location: ' . url('index.php?action=edit_trip&id=' . $tripId . '&error=1'));
            exit;
        }
        ...
        if (!$hasChanges) {
            header('Location: ' . url('index.php?action=my_trips&info=no_changes'));
            exit;
        }
        ...
        if ($result) {
            unset($_SESSION['trip_form_errors']);
            unset($_SESSION['trip_form_data']);
            header('Location: ' . url('index.php?action=my_trips&success=trip_updated'));
        } else {
    ...
    public function deleteTrip(): void
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            header('Location: ' . url('index.php?action=my_trips&error=invalid_request'));
            exit;
        }
        ...
        if (!$trip) {
            header('Location: ' . url('index.php?action=my_trips&error=trip_not_found'));
            exit;
        }
        ...
        // if (strtotime($trip['start_date']) < time()) {
        //     header('Location: ' . url('index.php?action=my_trips&error=past_trip_cannot_delete'));
        //     exit;
        // }
        ...
        if ($result) {
            header('Location: ' . url('index.php?action=my_trips&success=trip_deleted'));
        } else {
            header('Location: ' . url('index.php?action=my_trips&error=delete_failed'));
        }
        exit;
    }
}
