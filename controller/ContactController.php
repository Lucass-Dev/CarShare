...
class ContactController {
    ...
    private function validateSecurity($value, $fieldName = 'champ') {
        ...
        if (preg_match('/\\x00|\\0+|%00|\\x{0000}/i', $value)) {
            return false;
        }
...
<body>
    ...
    public function submit() {
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            header('Location: ' . url('index.php?action=contact&error=invalid_method'));
            exit();
        }
        ...
        if (empty($name) || empty($email) || empty($subject) || empty($message)) {
            header('Location: ' . url('index.php?action=contact&error=missing_fields'));
            exit();
        }
        ...
        if (!$this->validateSecurity($name, 'nom') || 
            ...
            header('Location: ' . url('index.php?action=contact&error=invalid_input'));
            exit();
        }
        
        // Validate name length and format
        if (strlen($name) < 2 || strlen($name) > 100) {
            header('Location: ' . url('index.php?action=contact&error=invalid_name'));
            exit();
        }
        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            header('Location: ' . url('index.php?action=contact&error=invalid_email'));
            exit();
        }
        
        // Validate subject length
        if (strlen($subject) < 3 || strlen($subject) > 200) {
            header('Location: ' . url('index.php?action=contact&error=invalid_subject'));
            exit();
        }

        if (strlen($message) < 10 || strlen($message) > 5000) {
            header('Location: ' . url('index.php?action=contact&error=message_length'));
            exit();
        }
        ...
        if ($emailSent) {
            header('Location: ' . url('index.php?action=contact&success=1'));
        } else {
            error_log('Échec de l\'envoi de l\'email de contact.');
            header('Location: ' . url('index.php?action=contact&error=email_error'));
        }
        exit();
    }
}
