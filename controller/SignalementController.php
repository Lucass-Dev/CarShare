...
class SignalementController
{
    ...
    public function render(): void
    {
        ...
        if (!$user) {
            header('Location: ' . url('index.php?action=signalement&error=user_not_found'));
            exit;
        }
    ...
    public function submit(): void
    {
        ...
        // Validation
        if (!$userId) {
            header('Location: ' . url('index.php?action=signalement&error=missing_data'));
            exit;
        }

        // Check that user is not reporting themselves
        if ($userId === $_SESSION['user_id']) {
            $redirectUrl = url('index.php?action=signalement&user_id=' . $userId . '&error=self_reporting');
            if ($carpoolingId) {
        ...
        if (empty($description)) {
            $redirectUrl = url('index.php?action=signalement&user_id=' . $userId . '&error=empty_description');
            if ($carpoolingId) {
                $redirectUrl .= '&carpooling_id=' . $carpoolingId;
            }
            header('Location: ' . $redirectUrl);
        ...
        if (empty($reason)) {
            $redirectUrl = url('index.php?action=signalement&user_id=' . $userId . '&error=empty_reason');
            if ($carpoolingId) {
                $redirectUrl .= '&carpooling_id=' . $carpoolingId;
            }
            header('Location: ' . $redirectUrl);
        ...
        if ($result) {
            $redirectUrl = url('index.php?action=signalement&user_id=' . $userId . '&success=1');
            if ($carpoolingId) {
                $redirectUrl .= '&carpooling_id=' . $carpoolingId;
            }
            header('Location: ' . $redirectUrl);
        } else {
            $redirectUrl = url('index.php?action=signalement&user_id=' . $userId . '&error=save_failed');
            if ($carpoolingId) {
                $redirectUrl .= '&carpooling_id=' . $carpoolingId;
            }
            header('Location: ' . $redirectUrl);
